    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-4 py-2 text-sm bg-(--color-accent) hover:bg-(--color-accent-hover) text-(--color-text-inverse) rounded-(--radius-md) transition-colors shadow-sm"
        >
        ...
        <div className="bg-(--color-bg-card) border border-(--color-border-default) rounded-(--radius-lg) p-5 shadow-(--shadow-card) space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <input
              ...
              className="px-3 py-2 bg-(--color-bg-input) border border-(--color-border-default) rounded-(--radius-md) text-sm text-(--color-text-primary) placeholder:text-(--color-text-muted) focus:border-(--color-border-focus) focus:outline-none focus:shadow-(--shadow-glow) transition-all"
            />
            <input
              type="text"
              value={manualPoster}
          ...
          <textarea
            ...
            rows={4}
            className="w-full px-3 py-2 bg-(--color-bg-input) border border-(--color-border-default) rounded-(--radius-md) text-sm text-(--color-text-primary) placeholder:text-(--color-text-muted) focus:border-(--color-border-focus) focus:outline-none focus:shadow-(--shadow-glow) resize-none transition-all"
          />
          <div className="flex gap-2">
            <button
              onClick={() => addMutation.mutate()}
              disabled={!manualText.trim() || addMutation.isPending}
              className="flex items-center gap-1.5 px-4 py-2 text-sm bg-(--color-accent) hover:bg-(--color-accent-hover) text-(--color-text-inverse) rounded-(--radius-md) transition-colors disabled:opacity-50 shadow-sm"
            >
      ...
      {isLoading && (
        <div className="space-y-3">
          {[...Array(5)].map((_, i) => (
            <div key={i} className="h-24 bg-(--color-bg-elevated) rounded-(--radius-lg) animate-pulse" />
          ))}
        </div>
      )}
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1.5">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-(--radius-full) font-medium ${
                      post.source === 'extension' ? 'bg-(--color-info-bg) text-(--color-info)' : 'bg-(--color-bg-elevated) text-(--color-text-secondary)'
                    }`}>
      ...
      {data && data.items.length === 0 && !isLoading && (
        <div className="text-center py-20">
          <div className="w-14 h-14 rounded-2xl bg-(--color-bg-elevated) flex items-center justify-center mx-auto mb-4">
            <Share2 className="w-7 h-7 text-(--color-text-muted)" />
          </div>
          <h3 className="text-base font-medium text-(--color-text-primary)">No LinkedIn posts yet</h3>
          <p className="text-sm text-(--color-text-muted) mt-1 max-w-md mx-auto">
