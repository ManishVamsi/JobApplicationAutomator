  ...
  return (
    <div className="bg-(--color-bg-card) border border-(--color-border-default) rounded-(--radius-lg) p-6 shadow-(--shadow-card)">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h2 className="text-lg font-semibold text-(--color-text-primary)">Resume</h2>
      </div>
      <p className="text-sm text-(--color-text-secondary) mb-4">
        Upload your resume (PDF or DOCX, max 10MB). The LLM will parse it to match jobs.
      </p>
      <label className="flex items-center justify-center gap-2 px-4 py-3 border-2 border-dashed border-(--color-border-default) rounded-(--radius-lg) cursor-pointer hover:border-(--color-accent)/50 hover:bg-(--color-accent-subtle) transition-all">
        {uploading ? (
    ...
    <div className="bg-(--color-bg-card) border border-(--color-border-default) rounded-(--radius-lg) p-6 shadow-(--shadow-card)">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-semibold text-(--color-text-primary)">Job Portals</h2>
        </div>
        <button
          onClick={() => setShowForm(!showForm)}
          className="flex items-center gap-1.5 px-3 py-1.5 text-sm bg-(--color-accent) hover:bg-(--color-accent-hover) text-white rounded-(--radius-md) transition-colors"
        >
      ...
      {showForm && (
        <div className="mb-4 p-4 bg-(--color-bg-primary) rounded-(--radius-md) border border-(--color-border-default) space-y-3">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="px-4 py-2 text-sm bg-(--color-accent) hover:bg-(--color-accent-hover) text-white rounded-(--radius-md) transition-colors disabled:opacity-50"
            >
      ...
      {isLoading ? (
        <div className="space-y-2">
          {[1, 2].map((i) => (
            <div key={i} className="h-14 bg-(--color-bg-primary) rounded-(--radius-md) animate-pulse" />
          ))}
        </div>
      ) : portals && portals.length > 0 ? (
        <div className="space-y-2">
          {portals.map((p) => (
            <div key={p.id} className="flex items-center justify-between px-4 py-3 bg-(--color-bg-primary) rounded-(--radius-md) border border-(--color-border-default)">
              ...
              <button
                ...
                className="p-1.5 text-(--color-text-muted) hover:text-(--color-error) hover:bg-(--color-error-subtle) rounded-(--radius-md) transition-all"
              >
  ...
  return (
    <div className="bg-(--color-bg-card) border border-(--color-border-default) rounded-(--radius-lg) p-6 shadow-(--shadow-card)">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h2 className="text-lg font-semibold text-(--color-text-primary)">Extension API Token</h2>
      </div>
      <p className="text-sm text-(--color-text-secondary) mb-4">
      ...
      {newToken && (
        <div className="mb-4 p-4 bg-(--color-success-subtle) rounded-(--radius-md) border border-(--color-success)/20">
          <p className="text-sm text-(--color-success) font-medium mb-2">
            ⚠️ Copy this token now — it won't be shown again!
          </p>
          <div className="flex items-center gap-2">
            <code className="flex-1 text-xs bg-(--color-bg-primary) px-3 py-2 rounded-(--radius-sm) font-mono text-(--color-text-primary) break-all select-all">
              {newToken}
            </code>
            <button
              onClick={handleCopy}
              className="p-2 bg-(--color-bg-primary) rounded-(--radius-md) hover:bg-(--color-bg-elevated) transition-colors"
            >
      ...
      {tokenInfo ? (
        <div className="flex items-center justify-between px-4 py-3 bg-(--color-bg-primary) rounded-(--radius-md) border border-(--color-border-default)">
          <div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1 px-3 py-1.5 text-xs bg-(--color-accent) hover:bg-(--color-accent-hover) text-white rounded-(--radius-md) transition-colors disabled:opacity-50"
            >
            ...
            <button
              ...
              className="flex items-center gap-1 px-3 py-1.5 text-xs text-(--color-error) bg-(--color-error-subtle) hover:bg-(--color-error)/20 rounded-(--radius-md) transition-colors"
            >
        ...
        <button
          onClick={() => generateMutation.mutate()}
          disabled={generateMutation.isPending}
          className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-(--color-accent) hover:bg-(--color-accent-hover) text-white font-medium rounded-(--radius-md) transition-colors disabled:opacity-50"
        >
  ...
  return (
    <div className="bg-(--color-bg-card) border border-(--color-border-default) rounded-(--radius-lg) p-6 shadow-(--shadow-card)">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h2 className="text-lg font-semibold text-(--color-text-primary)">Profile & Preferences</h2>
      </div>
      <div className="space-y-4">
        <div>
          ...
          <input
            ...
            className="w-full px-3 py-2 bg-(--color-bg-input) border border-(--color-border-default) rounded-(--radius-md) text-sm text-(--color-text-primary) placeholder:text-(--color-text-muted) focus:border-(--color-border-focus) focus:outline-none"
          />
        </div>
        <div>
          <label className="block text-sm text-(--color-text-secondary) mb-1">
            Target Roles <span className="text-(--color-text-muted)">(comma-separated)</span>
          </label>
          <input
            type="text"
            value={roles}
        ...
        <div>
          <label className="block text-sm text-(--color-text-secondary) mb-1">
            Target Locations <span className="text-(--color-text-muted)">(comma-separated)</span>
          </label>
          <input
            type="text"
            value={locations}
        ...
        <button
          onClick={handleSave}
          disabled={saving}
          className="flex items-center gap-2 px-4 py-2 bg-(--color-accent) hover:bg-(--color-accent-hover) text-white text-sm font-medium rounded-(--radius-md) transition-colors disabled:opacity-50"
        >
    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-(--color-text-primary) flex items-center gap-2">
          <Settings className="w-6 h-6 text-(--color-accent)" />
